...
`App\Services\StandaloneHotspotScriptGenerator` (2026-08-07) is a deliberately separate, unrelated code path — the "Script Generator" tool at `admin/tools/script-generator` (super admin only, gated by `abort_unless($request->user()->isSuperAdmin(), 403)` in `StandaloneScriptGeneratorController` the same way `TenantController`/`SecurityController` are). It generates a MikroTik hotspot config for routers that are **not**, and don't need to become, a `Router` record on this platform: no WireGuard, no RADIUS, no API user, no shop/tenant binding. Voucher codes become local `/ip hotspot user` accounts baked directly into the generated script (username and password both equal the code) instead of RADIUS-backed vouchers. It branches on RouterOS 6 vs 7 wireless syntax (`/interface wireless` vs `/interface wifi`/wifiwave2) and wireless-vs-wired-only hardware via plain capability toggles in the wizard (`App\Livewire\Admin\StandaloneScriptGenerator`, a 5-step form) rather than a hardcoded router-model catalog — deliberately, since a real model database would need ongoing upkeep as new MikroTik boards ship. The whole flow is stateless: nothing is persisted, the wizard's Livewire state holds everything, and the result page renders both the copyable/downloadable `.rsc` script and a printable voucher sheet (`voucher_template`: `grid` or `receipt`) in one response via `@media print` CSS — there was no reason to add a database table for something meant to be filled in, generated, and printed/copied in one sitting.
