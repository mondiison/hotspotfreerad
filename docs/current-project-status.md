...
- (2026-08-08) A third follow-up on the same feedback: bridged mode's bridge only ever had the wireless interface as a member -- no LAN port, so it extended nothing (glaring on single-Ethernet-port CPE hardware like the SXTsq, where that one port is the actual LAN connection). Added an optional `lan_port` per radio (default `ether1`, shown in the wizard only when link mode is bridged) -- when set, it's added as a second bridge port and explicitly enabled (`/interface ethernet set <port> disabled=no`); left blank, the script leaves a comment telling the admin to add one manually instead of silently shipping a bridge that does nothing.
- (2026-08-08) A fourth follow-up: the wireless interface itself was still coming up disabled on real hardware despite the inline `disabled=no` on the `set`/`configuration=` command -- RouterOS can reject mode/frequency changes on a still-disabled interface even when `disabled=no` is in that same command. `wirelessLines()` now issues an explicit `/interface wireless enable <interface>` (RouterOS 6) / `/interface wifi enable [find default-name=<interface>]` (RouterOS 7) as its own command first. See the `CLAUDE.md` note.
- (2026-08-08) A "make it professional" pass researched real MikroTik RouterOS wireless docs (help.mikrotik.com) after a live "bad band or frequency" rejection on a real RouterOS 6 router at a frequency well inside the card's own hardware range. Root cause: `band`/`country`/`frequency-mode` were never set at all, and RouterOS validates `frequency=` against those, not just raw hardware capability. Added: required `country` + per-radio `band` (options depend on that radio's own RouterOS version) + `frequency_mode` (`regulatory-domain` default, `superchannel` flagged as a compliance opt-in); `wireless_protocol` (`nv2` offered only when both radios are RouterOS 6 -- MikroTik's proprietary TDMA link protocol); `hide_ssid` (default on); `skip_dfs_channels` (RouterOS 7 only, default on); per-radio optional `antenna_gain_dbi` (RouterOS 6 only -- the RouterOS 7 property path couldn't be confirmed from documentation, so deliberately left unset there). Also fixed RouterOS 7's bridged-mode station role from plain `station` to `station-bridge`, per MikroTik's own docs on what actually supports being a bridge port. See the `CLAUDE.md` note for the full research-backed reasoning.
- (2026-08-09) Full dark-mode support added across the app. The admin panel gets a real Light/Dark/System toggle (Flux's `@fluxAppearance` dropdown pattern, in `admin.blade.php`'s header) since it was already wired up (localStorage + system-preference + flash-of-wrong-theme handling) but had zero `dark:` classes anywhere to make it visible. Auth pages, the public welcome/marketing page, and the tenant public site follow system preference only (no toggle UI) via `@fluxAppearance` with no menu. The customer-facing hotspot captive portal (`resources/views/hotspot/*.blade.php`) was deliberately left alone -- it's already a fixed `bg-zinc-950 text-white` dark brand aesthetic, not a light-mode default needing retrofit. Retrofit across ~55 files used a one-off Node script applying a light-token -> `dark:` sibling-class mapping (elevation hierarchy: body darkest `zinc-950` -> card/sidebar `zinc-900` -> subtle surface `zinc-800` -> borders `zinc-700`/`zinc-800`), followed by manual fixes for cases the script's mapping got wrong by being context-blind: standalone auth pages' `<body>` elevation (script had cards rendering darker than their own page background -- backwards), and "accent" UI (selected cards/tabs, primary buttons, progress-bar fills, file-input buttons, avatar chips) that uses `zinc-950` as a light-on-dark accent color rather than a body/surface tone, which needed inversion (`dark:bg-zinc-100 dark:text-zinc-900`) instead of the script's blind darkening. Full test suite confirmed clean against this work (see the pre-existing-failures note above and in `CLAUDE.md` for the ones that predate it).
