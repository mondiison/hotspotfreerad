...
- Paystack and Stripe (2026-08-06) got a read-only "Detected: Live/Test key" badge next to their Secret Key field instead of an editable Environment dropdown, since both only have one API host — the `sk_test_`/`sk_live_` key prefix is the real source of truth, so an independently-editable toggle there could disagree with the saved key. See the `PaymentGatewayCatalog::detectedEnvironment()` note in `CLAUDE.md`.
