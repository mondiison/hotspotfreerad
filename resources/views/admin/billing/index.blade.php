...
>
    <section class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 shadow-sm">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div class="min-w-0">
                    ...
                    <p class="mt-2 max-w-2xl text-sm leading-6 text-zinc-600 dark:text-zinc-400">{{ $platformGateway['summary'] }}</p>
                </div>
            </div>
        ...
        <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 shadow-sm">
            ...
            <p class="mt-1 text-sm leading-6 text-zinc-500 dark:text-zinc-400">Use these only on the MMS Radius platform gateway account.</p>
    ...
    @if (auth()->user()->isSuperAdmin())
        ...
        <section class="mt-6 overflow-hidden rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900">
            ...
            <table class="min-w-[760px] w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                    ...
                    @empty
                        <tr><td colspan="5" class="px-4 py-8 text-center text-zinc-500 dark:text-zinc-400">No platform billing subscriptions yet.</td></tr>
                    @endforelse
                </tbody>
            </table>
            </div>
        ...
        <section class="mt-6 overflow-hidden rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900">
            <div class="border-b border-zinc-200 dark:border-zinc-700 px-4 py-3">
                ...
                <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Tenant subscription checkout attempts and confirmations.</p>
            </div>
            <div class="overflow-x-auto overflow-y-hidden">
                    ...
                    <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                        @forelse ($platformPayments as $payment)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-right text-zinc-500 dark:text-zinc-400">{{ $payment->paid_at?->format('M j, Y g:i A') ?? '-' }}</td>
                                <td class="px-4 py-3 text-right">
                                    ...
                                    @else
                                        <span class="text-xs text-zinc-400 dark:text-zinc-500">Confirmed</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="7" class="px-4 py-8 text-center text-zinc-500 dark:text-zinc-400">No platform billing payments yet.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    @else
        <section class="mt-6 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-6">
            <p class="text-sm font-medium text-zinc-500 dark:text-zinc-400">{{ $tenant->company_name }}</p>
            ...
            <dl class="mt-6 grid gap-4 md:grid-cols-3">
                ...
                <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 p-4">
                    <dt class="text-sm text-zinc-500 dark:text-zinc-400">Monthly amount</dt>
                ...
                <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 p-4">
                    <dt class="text-sm text-zinc-500 dark:text-zinc-400">Renews</dt>
        ...
        <section class="mt-6 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-6">
            <div class="flex flex-col justify-between gap-3 md:flex-row md:items-start">
                <div>
                    ...
                    <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Pay the platform subscription from here. Hotspot customer collections remain on your shop's selected tenant gateway.</p>
                </div>
            ...
            <div class="mt-5 grid gap-4 md:grid-cols-3">
                @foreach ($plans as $plan)
                    <section class="rounded-lg border border-zinc-200 dark:border-zinc-700 p-4">
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                <h3 class="font-semibold">{{ $plan->name }}</h3>
                                <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">{{ $plan->currency }} {{ number_format($plan->monthly_price, 2) }} / month</p>
                            </div>
                ...
                <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                    @forelse ($subscriptions as $subscription)
                        <tr>
                            ...
                            <td class="px-4 py-3">{{ str_replace('_', ' ', ucfirst($subscription->status)) }}</td>
                            <td class="px-4 py-3">{{ $subscription->currency }} {{ number_format($subscription->amount, 2) }}</td>
                            <td class="px-4 py-3 text-zinc-500 dark:text-zinc-400">{{ optional($subscription->current_period_starts_at)->format('M j, Y') ?? 'Not started' }} - {{ optional($subscription->current_period_ends_at)->format('M j, Y') ?? 'Open' }}</td>
                        </tr>
                    @empty
                        <tr><td colspan="4" class="px-4 py-8 text-center text-zinc-500 dark:text-zinc-400">No billing history yet.</td></tr>
                    @endforelse
                </tbody>
            </table>
            </div>
        ...
        <section class="mt-6 overflow-hidden rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900">
            <div class="border-b border-zinc-200 dark:border-zinc-700 px-4 py-3">
                ...
                <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Your subscription checkout attempts on the platform account.</p>
            </div>
            <div class="overflow-x-auto overflow-y-hidden">
                <table class="min-w-[720px] w-full text-left text-sm">
                    ...
                    <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                        ...
                        @empty
                            <tr><td colspan="6" class="px-4 py-8 text-center text-zinc-500 dark:text-zinc-400">No platform billing payments yet.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
