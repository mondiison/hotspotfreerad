<x-layouts.admin title="Expenses" heading="Expenses" subheading="Track operating costs so sales reports can show practical profit.">
    @php($activeTab = request('tab') === 'categories' ? 'categories' : 'expenses')

    <x-slot:action>
        <div class="flex flex-wrap gap-2">
            @if ($activeTab === 'expenses')
                <flux:button href="{{ route('admin.expenses.export', request()->except('tab')) }}" variant="outline" icon="arrow-down-tray">Export CSV</flux:button>
    ...
    <div class="mb-6 flex flex-wrap gap-2 rounded-lg border border-zinc-200 bg-white p-2 shadow-sm">
        <flux:button
            href="{{ route('admin.expenses.index', request()->except(['tab', 'page'])) }}"
        ...
        <flux:button
            href="{{ route('admin.expenses.index', array_merge(request()->except(['page']), ['tab' => 'categories'])) }}"
    ...
    @else
        ...
        <section class="mt-6 flex flex-wrap gap-2">
            @foreach ($presets as $key => $label)
                <flux:button
                    href="{{ route('admin.expenses.index', array_merge(request()->except(['from', 'to', 'preset', 'page', 'tab']), ['preset' => $key])) }}"
            ...
            <flux:button
                href="{{ route('admin.expenses.index', array_merge(request()->except(['preset', 'page', 'tab']), ['from' => $filters['from'], 'to' => $filters['to']])) }}"
