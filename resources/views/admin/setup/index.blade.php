...
>
    <section class="rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 shadow-sm">
        <div class="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
            <div>
                <p class="text-sm font-medium text-zinc-500 dark:text-zinc-400">Launch Readiness</p>
        ...
        <div class="mt-5 h-2 overflow-hidden rounded-full bg-zinc-100 dark:bg-zinc-800">
            <div class="h-full rounded-full bg-zinc-950 dark:bg-zinc-100" style="width: {{ round(($completedCount / max(count($steps), 1)) * 100) }}%"></div>
        </div>
    </section>

    <section class="mt-6 grid gap-4 lg:grid-cols-3">
        @foreach ($steps as $step)
            <article class="flex min-h-64 flex-col rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 shadow-sm">
                <div class="flex items-start justify-between gap-3">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">{{ $step['phase'] }}</p>
                ...
                <p class="mt-4 flex-1 text-sm leading-6 text-zinc-600 dark:text-zinc-400">{{ $step['description'] }}</p>
                <p class="mt-4 rounded-md bg-zinc-50 dark:bg-zinc-800 p-3 text-xs leading-5 text-zinc-500 dark:text-zinc-400">{{ $step['hint'] }}</p>
    ...
    <section class="mt-6 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 shadow-sm">
        <div class="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
            <div>
                ...
                <p class="mt-2 max-w-3xl text-sm leading-6 text-zinc-600 dark:text-zinc-400">{{ $schedulerHealth['description'] }}</p>
            </div>
        ...
        <div class="mt-5 grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
            <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 p-4">
                <p class="text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Last heartbeat</p>
            ...
            <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 p-4">
                <p class="text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Pi cron entry</p>
    ...
    <section class="mt-6 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 shadow-sm" id="pppoe">
        <div class="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
            <div>
                <p class="text-sm font-medium text-zinc-500 dark:text-zinc-400">Provisioning Methods</p>
        ...
        <div class="mt-5 grid gap-4 xl:grid-cols-4">
            @foreach ($provisioningMethods as $method)
                <article class="rounded-lg border border-zinc-200 dark:border-zinc-700 p-4">
                    ...
                    <p class="mt-3 text-sm leading-6 text-zinc-600 dark:text-zinc-400">{{ $method['description'] }}</p>

                    <dl class="mt-4 space-y-3 text-xs leading-5">
                        <div>
                            <dt class="font-medium text-zinc-950 dark:text-zinc-100">Customer identity</dt>
                            <dd class="mt-1 text-zinc-500 dark:text-zinc-400">{{ $method['customer_identity'] }}</dd>
                        </div>
                        <div>
                            <dt class="font-medium text-zinc-950 dark:text-zinc-100">Router work</dt>
                            <dd class="mt-1 text-zinc-500 dark:text-zinc-400">{{ $method['router_work'] }}</dd>
                        </div>
                        <div>
                            <dt class="font-medium text-zinc-950 dark:text-zinc-100">Billing work</dt>
                            <dd class="mt-1 text-zinc-500 dark:text-zinc-400">{{ $method['billing_work'] }}</dd>
                        </div>
                    </dl>
    ...
    <section class="mt-6 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 shadow-sm">
        <div class="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
            <div>
                <p class="text-sm font-medium text-zinc-500 dark:text-zinc-400">PPPoE Wizard</p>
        ...
        <div class="mt-5 grid gap-4 lg:grid-cols-4">
            @foreach ($pppoeWizard as $step)
                <article class="rounded-lg border border-zinc-200 dark:border-zinc-700 p-4">
                    ...
                    <p class="mt-3 text-sm leading-6 text-zinc-600 dark:text-zinc-400">{{ $step['detail'] }}</p>
    ...
    <section class="mt-6 grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 shadow-sm">
            <div class="flex items-start justify-between gap-4">
                <div>
                    <p class="text-sm font-medium text-zinc-500 dark:text-zinc-400">Payment Readiness</p>
                    <h2 class="mt-2 text-xl font-semibold">Default gateway channels</h2>
                    <p class="mt-2 text-sm leading-6 text-zinc-600 dark:text-zinc-400">Customer money should go through tenant shop credentials. Each gateway can have its own keys, webhook secret, and settlement account.</p>
                </div>
            ...
            <div class="mt-5 space-y-3">
                @foreach ([
                    ...
                    <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 p-4">
                        ...
                        <p class="mt-2 text-xs leading-5 text-zinc-500 dark:text-zinc-400">{{ $channel['hint'] }}</p>
                    </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 shadow-sm">
            <div class="flex items-start justify-between gap-4">
                <div>
                    <p class="text-sm font-medium text-zinc-500 dark:text-zinc-400">Router Test</p>
                    <h2 class="mt-2 text-xl font-semibold">Captive portal checklist</h2>
                    <p class="mt-2 text-sm leading-6 text-zinc-600 dark:text-zinc-400">Use these checks after the RouterOS script and login.html template have been applied.</p>
                </div>
                @if ($firstRouter)
            ...
            <div class="mt-5 grid gap-3 md:grid-cols-2">
                @foreach ([
                    ...
                    <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 p-4 text-sm leading-6 text-zinc-600 dark:text-zinc-400">
    ...
    <section class="mt-6 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 shadow-sm">
        <div class="flex flex-col justify-between gap-3 md:flex-row md:items-center">
            <div>
                <p class="text-sm font-medium text-zinc-500 dark:text-zinc-400">Locations</p>
        ...
        <div class="mt-5 overflow-x-auto overflow-y-hidden rounded-lg border border-zinc-200 dark:border-zinc-700">
            <table class="min-w-[760px] w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                    @forelse ($shops as $shop)
                        <tr>
                            <td class="px-4 py-3">
                                ...
                                <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{{ $shop->tenant?->company_name }}{{ $shop->location_city ? ' - '.$shop->location_city : '' }}</p>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-8 text-center text-zinc-500 dark:text-zinc-400">Create a shop first, then continue with routers, packages, and payments.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
