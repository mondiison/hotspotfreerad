    ...
    <main class="flex min-h-screen items-center justify-center px-5 py-10">
        <section class="w-full max-w-md rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-6 shadow-sm">
            <div>
                ...
                <p class="mt-2 text-sm leading-6 text-zinc-600 dark:text-zinc-400">Open your authenticator app and enter the current 6-digit code. You can use a recovery code if your device is unavailable.</p>
            </div>
            ...
            <form method="POST" action="{{ route('two-factor.login') }}" class="mt-6 space-y-5">
                ...
                <label class="block">
                    ...
                    <input type="text" name="code" inputmode="numeric" autocomplete="one-time-code" class="mt-1 w-full rounded-md border border-zinc-300 dark:border-zinc-600 px-3 py-2" autofocus>
                    @error('code') <span class="text-sm text-red-600">{{ $message }}</span> @enderror
                </label>

                <label class="block">
                    <span class="text-sm font-medium">Recovery code</span>
                    <input type="text" name="recovery_code" autocomplete="one-time-code" class="mt-1 w-full rounded-md border border-zinc-300 dark:border-zinc-600 px-3 py-2">
