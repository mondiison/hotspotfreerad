...
<head>
    ...
    <title>Wi-Fi Access - {{ $shop->name }}</title>
    ...
    <main class="mx-auto flex min-h-screen w-full max-w-2xl flex-col justify-center px-5 py-8">
        <section class="rounded-lg border border-white/10 bg-white p-6 text-zinc-950">
            ...
            @else
                ...
                <h1 class="mt-2 text-2xl font-semibold">This device is trusted but not connected yet.</h1>
            ...
            <div class="mt-6 rounded-md bg-zinc-100 p-4 text-sm text-zinc-700">
                This network is for registered {{ strtolower($networkLabel) }} devices only. Extending access is handled by your administrator, not through this page.
            </div>
