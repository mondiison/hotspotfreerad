            ...
            <div class="max-h-96 divide-y divide-zinc-100 overflow-y-auto">
                @forelse ($notifications as $notification)
                    <a
                        ...
                        class="block px-4 py-3 text-sm hover:bg-zinc-50 {{ $notification->read_at ? '' : 'bg-blue-50/50' }}"
                    >
                        <div class="flex items-start gap-2">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="mt-0.5 text-xs text-zinc-500">{{ $notification->data['message'] ?? '' }}</p>
                                <p class="mt-1 text-xs text-zinc-400">{{ $notification->created_at->diffForHumans() }}</p>
                            </div>
                        </div>
                    </a>
                @empty
                    <p class="px-4 py-8 text-center text-sm text-zinc-500">No notifications yet.</p>
                @endforelse
            </div>
        </flux:popover>
    </flux:dropdown>
