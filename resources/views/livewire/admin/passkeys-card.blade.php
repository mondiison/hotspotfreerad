    ...
    <flux:card class="p-5">
        <div class="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(280px,360px)] lg:items-start">
            <div class="space-y-3">
                ...
                <flux:text class="text-sm text-zinc-500 dark:text-zinc-400">Register this browser or device as a passkey. Your device may ask for fingerprint, face unlock, PIN, or a hardware security key.</flux:text>
            ...
            <div x-data="passkeyManager()" class="rounded-md border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 p-4">
                <label class="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300">Passkey name</label>
                <input x-model="name" type="text" class="w-full rounded-md border-zinc-300 dark:border-zinc-600 text-sm" placeholder="Office laptop" />
                ...
                <div class="flex flex-col gap-3 px-5 py-4 md:flex-row md:items-center md:justify-between">
                    <div class="min-w-0">
                        <div class="flex flex-wrap items-center gap-2">
                            <div class="font-medium text-zinc-900 dark:text-zinc-100">{{ $passkey->name }}</div>
