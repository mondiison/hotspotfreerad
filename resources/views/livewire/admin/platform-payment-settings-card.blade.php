<form wire:submit="save" class="relative rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 shadow-sm">
    ...
    @if (! $snapshot['active_gateway_implemented'])
        <div class="mt-5 rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
            {{ $snapshot['active_gateway_name'] }} has no platform billing adapter yet -- credentials can still be saved here for planning, but tenant subscription checkout stays disabled on this gateway until one is added.
        </div>
    @endif
        ...
        @foreach ($gatewayCards as $key => $gateway)
            ...
            >
                <div class="flex items-center justify-between gap-3">
                    ...
                    <flux:badge color="{{ in_array($key, \App\Support\PaymentGatewayCatalog::platformImplementedGatewayKeys(), true) ? 'green' : 'amber' }}" size="sm">
                        {{ in_array($key, \App\Support\PaymentGatewayCatalog::platformImplementedGatewayKeys(), true) ? 'Platform adapter live' : 'Adapter pending' }}
                    </flux:badge>
                </div>
    ...
    <div class="mt-5 grid gap-4 md:grid-cols-2">
        <flux:field>
            ...
            <flux:description>Only gateways with a platform billing adapter (Flutterwave, Stripe, Monnify) can process checkout. Others can be selected for planning, but checkout stays disabled until an adapter is added.</flux:description>
        ...
        <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 p-4">
            <div class="flex items-center gap-3">
                ...
                <flux:badge color="{{ $snapshot['active_gateway_implemented'] ? 'green' : 'amber' }}" size="sm">
                    {{ $snapshot['active_gateway_implemented'] ? 'Platform adapter live' : 'Adapter pending' }}
                </flux:badge>
            </div>
            ...
            <flux:field>
                ...
                @if (array_key_exists($field, $selectFields))
                    ...
                    @if ($field === 'environment')
                        <flux:description>Sandbox/Test uses this gateway's test API and never moves real money. Switch to Live only once you have live platform credentials from {{ $activeGateway['name'] }}.</flux:description>
                    @endif
                @else
                    <flux:input
        ...
        @if ($active_gateway === 'flutterwave')
            <flux:field>
                ...
                <flux:description>Used for platform subscription checkout initialization. Only applies to Flutterwave's v4 orchestration API -- every other gateway's hosted checkout already offers its own method choice.</flux:description>
    ...
    @if ($hasStoredCredentials)
        <div class="mt-4 rounded-md border border-zinc-200 dark:border-zinc-700 p-3">
            <flux:checkbox wire:model.live="clear_gateway_credentials" label="Clear all saved {{ $activeGateway['name'] }} platform credentials" />
        </div>
    @endif
