                    ...
                    <tr wire:key="pos-device-{{ $device->id }}">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex justify-end gap-2">
                                <flux:button type="button" wire:click="renew({{ $device->id }})" wire:loading.attr="disabled" wire:target="renew({{ $device->id }})" variant="outline" size="sm" icon="arrow-path">
                                    <span wire:loading.remove wire:target="renew({{ $device->id }})">Renew</span>
                                    <span wire:loading wire:target="renew({{ $device->id }})">Renewing...</span>
                                </flux:button>
