<div>
    ...
    @elseif ($liveResource)
        <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <div class="rounded-lg border border-zinc-200 p-4">
                <p class="text-xs font-medium text-zinc-500 uppercase">CPU</p>
                <p class="mt-2 text-2xl font-semibold">{{ $liveResource['cpu_percent'] }}%</p>
            </div>
            <div class="rounded-lg border border-zinc-200 p-4">
                <p class="text-xs font-medium text-zinc-500 uppercase">RAM</p>
            ...
            <div class="rounded-lg border border-zinc-200 p-4">
                <p class="text-xs font-medium text-zinc-500 uppercase">Disk</p>
            ...
            <div class="rounded-lg border border-zinc-200 p-4">
                <p class="text-xs font-medium text-zinc-500 uppercase">Uptime</p>
        ...
        <p class="mt-3 text-xs text-zinc-500">{{ $liveResource['board_name'] }} &middot; RouterOS {{ $liveResource['version'] }}</p>
        ...
        @else
            <p class="mt-4 text-xs text-zinc-500">No hardware health sensors reported by this router (common on hardware without fans/temperature sensors).</p>
        @endif
    @endif
    ...
    @if (count($series) < 2)
        <p class="mt-4 py-8 text-center text-sm text-zinc-500">Not enough samples yet for this range. Samples are recorded every 5 minutes once the scheduler is running.</p>
    @else
        <div class="mt-4 grid gap-6 xl:grid-cols-2">
            <div>
                <p class="mb-2 text-sm font-medium text-zinc-600">CPU / RAM</p>
            ...
            <div>
                <p class="mb-2 text-sm font-medium text-zinc-600">Latency to router (ms)</p>
                <flux:chart :value="$series" class="aspect-[2/1]">
                    <flux:chart.svg>
    ...
    <p class="mt-4 text-xs text-zinc-500">Latency is measured by pinging this router's WireGuard IP directly &mdash; it doesn't need RouterOS API credentials. CPU/RAM/disk/health need the API section of the script applied on the router.</p>
</div>
