<div>
    ...
    <p class="mt-3 text-xs text-zinc-500">Scanning briefly interrupts client connections on some wireless drivers when run on an interface that's actively serving an SSID. Use the built-in Wi-Fi interface name from the router's script (for example <code>wifi1</code> on the L009 test profile).</p>
    ...
    @elseif ($hasScanned && empty($networks))
        <p class="mt-4 py-6 text-center text-sm text-zinc-500">No networks found in range.</p>
