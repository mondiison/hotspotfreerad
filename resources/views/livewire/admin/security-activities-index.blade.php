<div class="space-y-5">
                        ...
                        <tr wire:key="security-activity-{{ $activity->id }}">
                            <td class="px-4 py-3">
                                <div class="flex items-start gap-3">
                                    <span class="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        @endphp
                                        @if ($metadata !== '')
                                            <p class="mt-1 truncate text-xs text-zinc-400 dark:text-zinc-500">{{ $metadata }}</p>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-3">
                                ...
                                @else
                                    <span class="inline-flex items-center rounded-md bg-zinc-100 dark:bg-zinc-800 px-2 py-1 text-xs font-medium text-zinc-600 dark:text-zinc-400">Normal</span>
                                @endif
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 font-mono text-xs text-zinc-600 dark:text-zinc-400">{{ $activity->ip_address ?: '-' }}</td>
                            <td class="px-4 py-3 text-zinc-600 dark:text-zinc-400">
                                <p>{{ $activity->created_at->format('M j, Y H:i') }}</p>
                                <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{{ $activity->created_at->diffForHumans() }}</p>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-10 text-center">
                                ...
                                <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Try a wider date range or clear filters.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <flux:modal wire:model.self="showDetailModal" class="md:w-3xl" :dismissible="true">
        @if ($selectedActivity)
            <div class="space-y-5">
                <div class="flex items-start gap-3">
                    <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">{{ str($selectedActivity->action)->replace('_', ' ')->title() }} / {{ $selectedActivity->created_at->format('M j, Y H:i:s') }}</p>
                        <div class="mt-3">
                            ...
                            @else
                                <span class="inline-flex items-center rounded-md bg-zinc-100 dark:bg-zinc-800 px-2 py-1 text-xs font-medium text-zinc-600 dark:text-zinc-400">Normal activity</span>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="grid gap-3 md:grid-cols-2">
                    <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 p-4">
                        <p class="text-xs font-medium uppercase text-zinc-500 dark:text-zinc-400">Admin</p>
                        <p class="mt-2 font-medium text-zinc-950 dark:text-zinc-100">{{ $selectedActivity->user?->name ?? 'Deleted user' }}</p>
                        <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">{{ $selectedActivity->user?->email ?? '-' }}</p>
                    </div>

                    <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 p-4">
                        <p class="text-xs font-medium uppercase text-zinc-500 dark:text-zinc-400">Tenant</p>
                        <p class="mt-2 font-medium text-zinc-950 dark:text-zinc-100">{{ $selectedActivity->tenant?->company_name ?? 'Platform' }}</p>
                        <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">{{ $selectedActivity->tenant?->slug ?? '-' }}</p>
                    </div>

                    <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 p-4">
                        <p class="text-xs font-medium uppercase text-zinc-500 dark:text-zinc-400">IP Address</p>
                        <p class="mt-2 font-mono text-sm text-zinc-950 dark:text-zinc-100">{{ $selectedActivity->ip_address ?: '-' }}</p>
                    </div>

                    <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 p-4">
                        <p class="text-xs font-medium uppercase text-zinc-500 dark:text-zinc-400">Recorded</p>
                        <p class="mt-2 text-sm text-zinc-950 dark:text-zinc-100">{{ $selectedActivity->created_at->toDayDateTimeString() }}</p>
                    </div>
                </div>

                <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 p-4">
                    <p class="text-xs font-medium uppercase text-zinc-500 dark:text-zinc-400">User Agent</p>
                    <p class="mt-2 break-words text-sm text-zinc-700 dark:text-zinc-300">{{ $selectedActivity->user_agent ?: '-' }}</p>
                </div>

                <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 p-4">
                    <p class="text-xs font-medium uppercase text-zinc-500 dark:text-zinc-400">Metadata</p>
                    ...
                    @else
                        <p class="mt-2 text-sm text-zinc-500 dark:text-zinc-400">No additional metadata was recorded.</p>
                    @endif
                </div>
