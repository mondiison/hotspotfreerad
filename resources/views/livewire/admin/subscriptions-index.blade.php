    ...
    <div class="mt-6 overflow-x-auto overflow-y-hidden rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900">
        <table class="min-w-[1040px] w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                @forelse ($subscriptions as $subscription)
                    ...
                    <tr wire:key="subscription-{{ $subscription->id }}">
                        <td class="px-4 py-3">
                            ...
                            <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">Created {{ $subscription->created_at->diffForHumans() }}</p>
                        </td>
                        <td class="px-4 py-3">
                            <p class="font-medium">{{ $subscription->package?->name ?? 'Deleted package' }}</p>
                            <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{{ $subscription->package?->speed_limit_profile ?: 'No bandwidth profile' }}</p>
                        </td>
                        <td class="px-4 py-3">
                            <p>{{ $subscription->shop?->name ?? 'Deleted shop' }}</p>
                            <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{{ $subscription->shop?->tenant?->company_name }}</p>
                        </td>
                        <td class="px-4 py-3">
                            @if ($subscription->payment)
                                <flux:badge color="green">Paid</flux:badge>
                                <p class="mt-2 font-mono text-xs text-zinc-500 dark:text-zinc-400">{{ $subscription->payment->tx_ref }}</p>
                            @else
                                <flux:badge color="sky">Test</flux:badge>
                                <p class="mt-2 text-xs text-zinc-500 dark:text-zinc-400">No payment attached</p>
                            @endif
                        </td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3 text-right">
                            @if ($usage['available'])
                                ...
                                <p class="mt-1 text-xs text-zinc-400 dark:text-zinc-500">{{ number_format($usage['session_count']) }} RADIUS {{ \Illuminate\Support\Str::plural('session', $usage['session_count']) }}</p>
                            @else
                        ...
                        <td class="px-4 py-3 text-right">
                            ...
                            <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">Started {{ $subscription->starts_at->format('M j, Y g:i A') }}</p>
                        </td>
                        <td class="px-4 py-3 text-right">
                ...
                @empty
                    <tr><td colspan="8" class="px-4 py-8 text-center text-zinc-500 dark:text-zinc-400">No access records found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
            ...
            <div class="space-y-6">
                ...
                <section class="grid gap-3 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-4 text-sm md:grid-cols-2">
                    <div>
                        ...
                        <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{{ $selectedSubscription->package?->speed_limit_profile ?: 'No bandwidth profile' }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        <p class="text-xs font-medium uppercase text-zinc-500 dark:text-zinc-400">Source</p>
                        <p class="mt-1 font-medium">{{ $selectedSubscription->payment ? 'Paid' : 'Test' }}</p>
                        @if ($selectedSubscription->payment)
                            <p class="mt-1 font-mono text-xs text-zinc-500 dark:text-zinc-400">{{ $selectedSubscription->payment->tx_ref }}</p>
                        @endif
                    </div>
                    <div>
                ...
                <section class="overflow-hidden rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900">
                    <div class="border-b border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 px-4 py-3">
                        ...
                        <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">Only sessions whose start time falls inside this access window are included.</p>
                    </div>
                                ...
                                @forelse ($sessions as $session)
                                    <tr>
                                        ...
                                        <td class="px-4 py-3 text-zinc-600 dark:text-zinc-400">
                                            ...
                                            <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{{ $session->acctstoptime ? 'Stopped '.\Illuminate\Support\Carbon::parse($session->acctstoptime)->format('M j, Y g:i A') : 'Still open / no stop' }}</p>
                                        </td>
                                        <td class="px-4 py-3 text-right">
