<div>
    ...
    <form wire:submit.prevent="save" class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div class="space-y-6">
            <section class="rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-6">
                <div>
                    <h2 class="text-base font-semibold">Public Page Copy</h2>
                    <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">These values control the tenant slug page: {{ $tenant->publicUrl() }}</p>
                </div>
            ...
            <section class="rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-6">
                <div>
                    ...
                    <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Upload public-facing images. Use compressed JPG, PNG, or WebP files under 4MB each.</p>
                </div>
        ...
        <aside class="space-y-4">
                ...
                <div class="mt-4 overflow-hidden rounded-lg border border-zinc-200 dark:border-zinc-700">
                    <div class="p-4 text-white" style="background-color: {{ $brand_color }}">
                        <div class="flex items-center gap-3">
                            @if ($tenant->logo_image_path)
                                <img src="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($tenant->logo_image_path) }}" alt="{{ $tenant->company_name }} logo" class="h-10 w-10 rounded-md bg-white dark:bg-zinc-900 object-cover">
                            @else
                    ...
                    <div class="bg-zinc-50 dark:bg-zinc-800 p-4 text-sm text-zinc-600 dark:text-zinc-400">
            ...
            <section class="rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-5 text-sm text-zinc-600 dark:text-zinc-400">
                <h2 class="text-base font-semibold text-zinc-950 dark:text-zinc-100">Branding Guide</h2>
