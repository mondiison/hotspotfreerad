        ...
        <div class="min-h-screen">
            <aside class="fixed inset-y-0 left-0 hidden w-64 border-r border-white/10 bg-zinc-950/95 px-5 py-6 lg:block">
                <div class="flex items-center gap-3">
                    <div class="grid size-9 place-items-center rounded-lg bg-emerald-400 text-sm font-bold text-zinc-950 dark:text-zinc-100">HF</div>
                    <div>
                        <p class="text-sm font-semibold">HotspotFreeRAD</p>
                        <p class="text-xs text-zinc-400 dark:text-zinc-500">Tenant operations</p>
                    </div>
                </div>
                ...
                <nav class="mt-8 space-y-1 text-sm">
                    ...
                    <a class="flex items-center gap-3 rounded-lg px-3 py-2 text-zinc-400 dark:text-zinc-500 hover:bg-white/5 hover:text-white" href="#">
                ...
                <section class="px-5 py-6 sm:px-8">
                    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                        <div class="rounded-lg border border-white/10 bg-white/[0.03] p-5">
                            <p class="text-sm text-zinc-400 dark:text-zinc-500">Tenants</p>
                            <p class="mt-3 text-3xl font-semibold text-white">0</p>
                            <p class="mt-2 text-xs text-zinc-500 dark:text-zinc-400">Ready for onboarding flow</p>
                        </div>

                        <div class="rounded-lg border border-white/10 bg-white/[0.03] p-5">
                            <p class="text-sm text-zinc-400 dark:text-zinc-500">Routers online</p>
                            <p class="mt-3 text-3xl font-semibold text-white">0</p>
                            <p class="mt-2 text-xs text-zinc-500 dark:text-zinc-400">WireGuard and RADIUS pending</p>
                        </div>

                        <div class="rounded-lg border border-white/10 bg-white/[0.03] p-5">
                            <p class="text-sm text-zinc-400 dark:text-zinc-500">Active sessions</p>
                            <p class="mt-3 text-3xl font-semibold text-white">0</p>
                            <p class="mt-2 text-xs text-zinc-500 dark:text-zinc-400">FreeRADIUS accounting not connected</p>
                        </div>

                        <div class="rounded-lg border border-white/10 bg-white/[0.03] p-5">
                            <p class="text-sm text-zinc-400 dark:text-zinc-500">Today revenue</p>
                            <p class="mt-3 text-3xl font-semibold text-white">NGN 0</p>
                            <p class="mt-2 text-xs text-zinc-500 dark:text-zinc-400">Gateway checkout flow coming next</p>
                        </div>
                    </div>
                            ...
                            <div class="divide-y divide-white/10">
                                @foreach ([
                                    ...
                                    <div class="flex gap-4 px-5 py-4">
                                        <div class="mt-0.5 grid size-7 shrink-0 place-items-center rounded-full {{ $item['done'] ? 'bg-emerald-400 text-zinc-950 dark:text-zinc-100' : 'bg-white/10 text-zinc-500 dark:text-zinc-400' }}">
                                            <flux:icon.check class="size-4" />
                                        </div>
                                        <div>
                                            <p class="text-sm font-medium text-white">{{ $item['label'] }}</p>
                                            <p class="mt-1 text-sm text-zinc-400 dark:text-zinc-500">{{ $item['detail'] }}</p>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="rounded-lg border border-white/10 bg-zinc-900/60 p-5">
                            ...
                            <div class="mt-4 space-y-4">
                                <div class="rounded-lg bg-white/[0.04] p-4">
                                    ...
                                    <p class="mt-1 text-sm text-zinc-400 dark:text-zinc-500">Register the ISP owner and subscription plan.</p>
                                </div>
                                <div class="rounded-lg bg-white/[0.04] p-4">
                                    <p class="text-sm font-medium text-white">2. Add shop and router</p>
                                    <p class="mt-1 text-sm text-zinc-400 dark:text-zinc-500">Generate MikroTik setup from NAS ID, shared secret, and WireGuard IP.</p>
                                </div>
                                <div class="rounded-lg bg-white/[0.04] p-4">
                                    <p class="text-sm font-medium text-white">3. Publish packages</p>
                                    <p class="mt-1 text-sm text-zinc-400 dark:text-zinc-500">Define price, session time, rate limit, and FUP thresholds.</p>
                                </div>
                            </div>
                        </div>
                    </div>
